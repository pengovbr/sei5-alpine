...
RUN apk add --no-cache \
    ...
    gearman-libs \
    gearman-dev \
    && pecl82 install gearman \
    && echo "extension=gearman.so" > /etc/php82/conf.d/50_gearman.ini \
    && apk del \
        gearman-dev \
        gcc \
        g++ \
...
# wkhtmltopdf #
COPY --from=surnet/alpine-wkhtmltopdf:3.23.4-0.12.6-small \
